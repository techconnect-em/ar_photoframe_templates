...
// Crown / Face Detection state
let faceLandmarker = null;
let crownImage = null;
let crownCanvas = null;
let crownCtx = null;
let lastCrownState = null;
...
async function initFaceLandmarker() {
  try {
    ...
    console.warn('FaceLandmarker initialization failed:', error);
    faceLandmarker = null;
  }
}
...
function loadCrownImage() {
  return new Promise((resolve) => {
    ...
    img.onerror = () => {
      console.warn('Crown image failed to load:', CROWN_IMAGE_PATH);
      crownImage = null;
      resolve(null);
    };
...
function runFaceDetectionLoop() {
  ...
  const now = Date.now();
  const interval = 1000 / FACE_DETECTION_FPS;
...
function detectAndDrawCrown() {
  if (!faceLandmarker || !crownImage || !cameraEl || cameraEl.readyState < 2) {
    clearCrown();
    return;
  }
  ...
  try {
    ...
    const transform = computeCrownTransform(landmarks);
...
function computeCrownTransform(landmarks) {
  // Key landmarks for crown positioning
  // 10: forehead center (top of face)
  // 234: left temple, 454: right temple (for face width)
  // 33: left eye outer, 263: right eye outer (for rotation)
  ...
  if (!forehead || !leftTemple || !rightTemple || !leftEye || !rightEye) {
    return null;
  }

  // Convert normalized coordinates to canvas coordinates
  const canvasWidth = crownCanvas.width;
  const canvasHeight = crownCanvas.height;
...
function smoothTransform(current) {
  ...
  return {
    x: lerp(lastCrownState.x, current.x, SMOOTHING_FACTOR),
    y: lerp(lastCrownState.y, current.y, SMOOTHING_FACTOR),
    width: lerp(lastCrownState.width, current.width, SMOOTHING_FACTOR),
    height: lerp(lastCrownState.height, current.height, SMOOTHING_FACTOR),
    angle: lerpAngle(lastCrownState.angle, current.angle, SMOOTHING_FACTOR),
  };
}
...
function drawCrown(transform) {
  if (!crownCtx || !crownImage) return;

  crownCtx.clearRect(0, 0, crownCanvas.width, crownCanvas.height);

  crownCtx.save();
  crownCtx.translate(transform.x, transform.y + transform.height / 2);
  crownCtx.rotate(transform.angle);
  crownCtx.drawImage(
    crownImage,
    -transform.width / 2,
    -transform.height / 2,
...
function updateCrownCanvasSize() {
  if (!crownCanvas || !previewEl) return;

  const rect = previewEl.getBoundingClientRect();
  if (crownCanvas.width !== rect.width || crownCanvas.height !== rect.height) {
    crownCanvas.width = rect.width;
    crownCanvas.height = rect.height;
  }
}

// ==================== End Crown / Face Detection ====================
...
function composeScreenshot() {
  ...
  // Draw crown overlay (between camera and frame)
  if (crownCanvas && lastCrownState && crownImage) {
    try {
      ctx.drawImage(crownCanvas, 0, 0, outputWidth, outputHeight);
    } catch (error) {
      console.warn('王冠の描画に失敗しました:', error);
    }
  }
...
async function init() {
  ...
  if (crownCanvas) {
    ...
    updateCrownCanvasSize();
    window.addEventListener('resize', updateCrownCanvasSize);
  }
  ...
  // Load crown image and initialize face detection (non-blocking)
  loadCrownImage().then(() => {
